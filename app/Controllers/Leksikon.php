...
class Leksikon extends BaseController
{
    public function index()
    {
        ...
        if (!$session->has('wotd_id')) {
            ...
            if ($kata_hari_ini) $session->set('wotd_id', $kata_hari_ini['id']);
        } else {
            $kata_hari_ini = $entriModel->find($session->get('wotd_id')) ?? $entriModel->orderBy('RAND()')->first();
            if ($kata_hari_ini) $session->set('wotd_id', $kata_hari_ini['id']);
        }

        $kuis_soal = null; $kuis_pilihan = [];
        ...
        if (!in_array($sortBy, $allowedSortColumns)) $sortBy = 'term';
    ...
    public function search()
    {
        ...
        $results = $entriModel
            ...
            ->like('term', $q, 'both') // Use 'both' for %$q%
            ->orLike('definition', $q, 'both') // Use 'both' for %$q%
            ->limit(5)
            ->findAll();
    ...
    public function check_quiz()
    {
        ...
        if (!empty($jawaban_user) && !empty($jawaban_benar)) {
            if ($jawaban_user === $jawaban_benar) {
                ...
                $session->setFlashdata('notif_kuis', ['tipe' => 'danger', 'pesan' => "Jawaban salah. Yang benar: " . htmlspecialchars($jawaban_benar)]);
            }
        }
